...
typedef struct _ccd_vec2_t ccd_vec2_t;

_ccd_inline ccd_real_t ccdVec2Dot(const ccd_vec2_t *a, const ccd_vec2_t *b)
{
    ccd_real_t dot;
...
_ccd_inline void ccdVec2Scale(ccd_vec2_t *d, ccd_real_t k)
{
    d->v[0] *= k;
    d->v[1] *= k;
}

_ccd_inline ccd_real_t ccdVec2Len2(const ccd_vec2_t *v)
{
    return ccdVec2Dot(v, v);
}

_ccd_inline void ccdVec2Normalize(ccd_vec2_t *d)
{
    ccd_real_t k = CCD_ONE / CCD_SQRT(ccdVec2Len2(d));
    ccdVec2Scale(d, k);
}
#ifdef __cplusplus
} /* extern "C" */
